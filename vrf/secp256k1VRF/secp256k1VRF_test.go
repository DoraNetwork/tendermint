...
import (
	...
	"github.com/tendermint/tendermint/vrf/bitelliptic"
)
...
func TestProofToHash(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		bytes, _ := utils.FromHex(privKey)
		k, err := NewVRFSignerFromRawKey(bytes)
		idx, proof := k.Evaluate(tc.m)
		fmt.Println("=======")
		fmt.Println(utils.Hex(idx[0:]))
		fmt.Println(utils.Hex(proof))
...
func Test256VRF(t *testing.T) {
	...
	seckey, err := utils.FromHex(privKey)
	if err != nil {
		t.Errorf("load priv err: %v", err)
	}
	ecdsaPriv, err := utils.ToECDSAPrivateKey(bitelliptic.S256(), seckey)
	if err != nil {
	...
	seckeyPub, err := utils.FromHex(pubKey)
